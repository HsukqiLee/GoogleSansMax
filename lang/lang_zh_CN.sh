...
TXT_XML_ALIAS_FALLBACK="⚠️ 挂载安全降级：当前为 %3\$s，且原厂 /system/%2\$s 是符号链接；已跳过 UFS 自己在 %1\$s 的字体 XML 覆盖，避免 OverlayFS 将该链接遮蔽为真实目录。UFS 不识别或猜测 metamodule / magic mount 能力；其它安全路径继续处理。如已确认当前挂载实现能正确处理特殊分区，可将 SPECIAL_PARTITION_XML_MODE 设为 force"
TXT_XML_ALIAS_TOPOLOGY_DEFERRED="⚠️ 挂载安全降级：当前为 %3\$s，但尚未取得原厂 /system/%2\$s 的路径类型；safe 模式本次不创建 UFS 自己的 %1\$s 目录，将在下次 pre-mount 重建时重新判断。如已确认当前挂载实现能正确处理特殊分区，可将 SPECIAL_PARTITION_XML_MODE 设为 force"
TXT_XML_UNSAFE_RESIDUAL="❌ 挂载安全检查未通过：%s 无法由 UFS 安全清理；为避免 OverlayFS 下遮蔽系统符号链接，已停止本次字体 XML 重建。请检查该路径是否包含非 UFS 文件或符号链接"
TXT_XML_SPECIAL_MODE_FORCE="⚠️ 特殊分区 XML 模式为 force：KernelSU / APatch 下将允许 UFS 使用标准 Magisk 布局写入 product、system_ext 等特殊分区 XML，即使原厂 /system/<partition> 是符号链接。请仅在已确认当前 magic mount / OverlayFS metamodule 能正确处理这些分区时使用，否则可能导致启动故障"
TXT_XML_SPECIAL_MODE_INVALID="⚠️ SPECIAL_PARTITION_XML_MODE=%s 无效，已回退到 safe 模式"
