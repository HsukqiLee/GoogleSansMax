...
monitor_font_modules() {
    ...
    if ufs_rebase_all_xml "$print_func"; then
        xml_actions="${UFS_XML_REBASE_ACTIONS:-0}"
    else
        "$print_func" "$(safe_text TXT_XML_REBASE_FAILED)"
    fi
    ...
    if [ "$xml_actions" -eq 0 ] && [ "$found_binary_actions" -eq 0 ]; then
